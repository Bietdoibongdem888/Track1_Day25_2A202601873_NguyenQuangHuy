...
|---|---|---|---|
| The old 80% value was only described as an estimate and was vulnerable to containment terminology drift. | ACCEPT | P-015 has five directly relevant deterministic E2E reports; derive a reproducible package-completion record without using unrelated chat queries. | Added `containment-eval.md`, `containment-eval.csv` and `build_containment_eval.py`; model now labels 80% as commercial package completion, 20% as autonomous containment, 80% as human-review/escalation and 20% as grounding failure. |
